...
def main(args):
    ...
    with mlflow.start_run():
        ...
        processed_path = os.path.join(project_root, "data", "processed", "encuesta_fecundidad_mujeres.csv")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Run Fecundity pipeline with XGBoost + MLflow")
    p.add_argument("--input", type=str, required=True,
                   help="path to CSV (e.g., data/raw/***.csv)")
    p.add_argument("--target", type=str, default="NHIJOBIO")
    p.add_argument("--threshold", type=float, default=0.35)
    p.add_argument("--test_size", type=float, default=0.2)
    p.add_argument("--experiment", type=str, default="INE 2018")
