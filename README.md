## Fertility Prediction for Women – End-to-End ML Project
### Purpose

Build and ship a full machine-learning solution for predicting fertility outcomes in women—from data prep and modeling to an API + web UI deployed on AWS.

### Problem solved & benefits

- Earlier insights: Predicts fertility potential so healthcare providers and patients can make informed decisions about family planning and treatment options.
- Operationalized ML: Model is accessible via a REST API and a simple UI; clinicians and patients can access predictions without technical expertise.
- Repeatable delivery: CI/CD + containers mean every change can be rebuilt, tested, and redeployed in a consistent way.
- Traceable experiments: MLflow tracks runs, metrics, and artifacts for reproducibility and clinical auditing.
